...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    _ss_exp = (fk[fk["ExpiryStatus"] == "Expired"]
                  .groupby("Store_Name", observed=True)["QuantityOnHand"]
                  .sum().reset_index().rename(columns={"QuantityOnHand": "ExpiredQty"}))
    # BUG FIX: all three were grouping by ProductName which has only 6 unique values
    # → every store showed 6 for NearExpiry and undercount for OOS/LowStock.
    # Now using ProductID (unique SKU) for correct counts.
    _ss_near = (fk[fk["DaysToExpiry"].between(0, 30)]
                   .groupby("Store_Name", observed=True)["ProductID"]
                   .nunique().reset_index().rename(columns={"ProductID": "NearExpiry"}))
    _ss_oos = (fk[fk["QuantityOnHand"] == 0]
                  .groupby("Store_Name", observed=True)["ProductID"]
                  .count().reset_index().rename(columns={"ProductID": "OOS_Items"}))
    _ss_low = (fk[fk["QuantityOnHand"] < fk["ReorderPoint"]]
                  .groupby("Store_Name", observed=True)["ProductID"]
                  .count().reset_index().rename(columns={"ProductID": "LowStock"}))
    _ss_sv = (fk.copy()
    ...
    for _col in ["ExpiredQty", "NearExpiry", "OOS_Items", "LowStock", "Transactions", "QuantitySold"]:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption(
        "Try: *Which medicines need reordering? · Which supplier is underperforming? "
        "· Which category has highest expiry risk? · How can revenue improve?*"
    )
    ...
    for _msg in _chat_history:
        if _msg["role"] == "user":
            st.markdown(
                f'<div class="ai-chat-card"><b>You:</b> {_msg["content"]}</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="ai-response-card">{_msg["content"].replace(chr(10), "<br>")}</div>',
                unsafe_allow_html=True,
            )

    user_q = st.text_input(
        "Your question",
        placeholder="Ask AI about your pharmacy data…",
        key="ai_chat_input",
    ...
    with _btn_col1:
        ask_btn = st.button("➤ Ask", key="btn_ask_ai", type="secondary", use_container_width=True)
    with _btn_col2:
    ...
    if ask_btn and user_q.strip():
        # Build flat tuple of alternating (user, assistant) strings for cache key
        _flat_history = tuple(
            m["content"] for m in _chat_history
        )
